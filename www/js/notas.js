...
var _notaObrigatoriaId = 'nota_marcas_petz';

// Nota obrigatoria que sempre deve existir
var _notaObrigatoria = {
    id: _notaObrigatoriaId,
    titulo: 'Marcas Próprias PETZ',
    conteudo: 'Fuzz\nHappy Med\nPetz\nFresh\nNutrifresh\nPetix\nSelections\nPatas Secas\nCansei de Ser Gato\nSpik',
    cor: '#dbeafe',
    fixada: true,
    lembrete: false,
...
// Garante que a nota obrigatoria existe no localStorage
function notasGarantirObrigatoria() {
    var notas = notasObterTodas();
    var existe = notas.find(function(n) { return n.id === _notaObrigatoriaId; });
    if (!existe) {
        notas.unshift(_notaObrigatoria);
        notasSalvarTodas(notas);
    } else {
        // Atualizar conteudo caso tenha sido alterado
        var atualizar = false;
        if (existe.titulo !== _notaObrigatoria.titulo) { existe.titulo = _notaObrigatoria.titulo; atualizar = true; }
        if (existe.conteudo !== _notaObrigatoria.conteudo) { existe.conteudo = _notaObrigatoria.conteudo; atualizar = true; }
        if (!existe.obrigatoria) { existe.obrigatoria = true; atualizar = true; }
        if (atualizar) notasSalvarTodas(notas);
    }
}
...
function notasRenderizar() {
    ...
    filtradas.forEach(function(nota) {
        ...
        if (!nota.obrigatoria) {
            ...
            html += '      <span style="font-size:10px;color:#6b7280;padding:4px 8px;">Obrigatória</span>';
        }
        html += '    </div>';
        html += '  </div>';
